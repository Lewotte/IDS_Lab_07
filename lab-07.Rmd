...
From simple inspection, the distribution of score shows no skewness when plotted againstcourse ID.
...
The scatter plot in exercise 2 is misleading because repeating data values cover each other up, appearing as only one data value.

Jitter, meaning slight irregular variation, nudges each data value randomly so that we can see multiple data points are mapped to the same x,y position.
...
tidy(score_bty_fit)
```
The linear model would be $\hat{y} = 3.88033795 + 0.06663704x$
...
The positive slope suggests professors with higher average beauty ratings generally receive higher evaluation score, by the increment of 0.067 in score per 1 increase in average beauty ratings.
